...
module.exports = {
    checkAuth: path => {
        return async (req, res, next) => {
            if (!req.user) await Log.handleError(path, req, res, { code: 401 });
            else next();
        };
    },
    checkAuthBasic: path => {
    ...
    checkAdmin: path => {
        return async (req, res, next) => {
            if (!req.user)
                await Log.handleError(path, req, res, { code: 401, message: "not authorized" });
            else if (req.user.role !== "admin")
                await Log.handleError(path, req, res, { code: 403, message: "forbidden" });
            else next();
        };
    },
    checkTeacher: path => {
